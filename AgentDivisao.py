...
class DivisaoAgent(Agent):
    class DivisaoBehav(CyclicBehaviour):
        async def on_start(self):
            print("Starting DIVISÃO behaviour . . .")
            

        async def run(self):
            # Espere pela mensagem por 10 segundos
            msg = await self.receive(timeout=10)#Recebendo a mensagem
            print("Mensagem recebida do Coordenador" + msg.body)
    ...
    async def setup(self):
        print("Agent DIVISÃO starting . . .")
        my_behav = self.DivisaoBehav()
...
if __name__ == "__main__":
    dummy = DivisaoAgent("agent3Divisao@anoxinon.me", "divisao")
    future = dummy.start()                                           
